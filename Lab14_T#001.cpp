...
void bubbleSort(int arr[], int n)
{
    for (int i = 0; i < n - 1; i++)
    {
        ...
        {
            if (arr[j] < arr[j + 1])  // Changed comparison to '<' for descending order
            {
                int temp = arr[j];
                arr[j] = arr[j + 1];
                arr[j + 1] = temp;
...
void insertionSort(int arr[], int n)
{
    for (int i = 1; i < n; i++)
    {
        ...
        while (j >= 0 && arr[j] < key)  // Changed comparison to '<' for descending order
        {
            arr[j + 1] = arr[j];
            j = j - 1;
        }
...
int main()
{
    ...
    cout << "\nBubble Sorted Array (Descending): ";
    ...
    cout << "\nInsertion Sorted Array (Descending): ";
